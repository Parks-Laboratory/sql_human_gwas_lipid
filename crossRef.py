...
os.mkdir('E:/cross_ref/jgrot/masterOutput/output')
...
def sqlSearchString(geneDF, pval):
    ...
    if len(errList) > 0:
        print("\nGene(s): " + str(errList) + " not found in HG19_ensembl table")
    i = 0
    while i < geneCount:
...
def tableLoad(stringList, geneNameList):
    ...
    while j < len(stringList):
            ...
            path = 'E:/cross_ref/jgrot/masterOutput/output/%s.txt' %(geneNameList[j])
