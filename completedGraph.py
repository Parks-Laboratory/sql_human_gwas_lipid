...
4. Nothing:\n""")
    ...
    try:
        if option == '1':
            ...
            traits = df.trait
            pVal = df.p_value.min()
            pValLog = df['p_value_log'] = abs(np.log10(df['p_value'])) 
            grouped = df.groupby("table_name")
            plotInput = input("What table(s) are you plotting? \nPress ENTER to include all tables. \nOtherwise, enter the table numbers that you want to include seperated by a comma e.g.) 1,3,5\n")

            if len(plotInput)==1:
                graph = plt.bar(traits, pValLog)  
                plt.title('Trait vs P-Value')  
                plt.xlabel('Trait')
                plt.ylabel('P-Value')
                plt.show()

            elif ',' in plotInput:
                multPlot = list(set([int(i) for i in plotInput.split(",")]))
               # selectGrouped = multPlot.groupby("table_name")
                for i in multPlot:
                    plt.figure()
                    plt.bar(traits[i], pValLog[i])
                    plt.title("Trait vs P-Value: Table "+ str(i))
                    plt.xlabel('Trait')
                    plt.ylabel('P-Value')
                    plt.show()
                ...
                for i in range (len(grouped)):
                    plt.figure()
                    graph = plt.bar(traits[i], pValLog[i])  
                    plt.title("Trait vs P-Value: Table " + str(i+1))  
                    plt.xlabel('Trait')
                    plt.ylabel('P-Value')
                    plt.show()  
