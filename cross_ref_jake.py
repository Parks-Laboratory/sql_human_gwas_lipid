...
3. Nothing:\n""")
    ...
    try:
        ...
        if option == '2':
           # engine = create_engine('mssql+pyodbc://@PARKSLAB')
           # engine = sqlalchemy.create_engine('mssql://PARKSLAB/Human_GWAS?trusted_connection=yes') // 2 iteration

            engine = create_engine('mssql+pyodbc://@' + 'PARKSLAB' + '/' + 'Human_GWAS' + '?trusted_connection=yes&driver=ODBC+Driver+11+for+SQL+Server')
            df.to_sql(name='cross_ref_lipid_human_gwas-3', schema='dbo', con=engine, if_exists='replace',index=False) #believe this works...error is no longer thrown..has loading period



        #      params = 'DRIVER={ODBC Driver 11 for SQL Server};' \ // param variable setup for the first iteration of solving 'option 2'
        #  'SERVER=PARKSLAB;' \
        #  'PORT=1434;' \
        #  'DATABASE=Human_GWAS;' \
        #  'UID=jgrothaus;' \
        ...
        # df.to_sql(name='cross_ref', schema='dbo', con=db, if_exists='replace',index=False) // error messages return with the adddition of this line... progress? or worse?
