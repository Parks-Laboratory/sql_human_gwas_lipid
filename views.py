...
os.mkdir('E:/BRIAN/Cross_Ref_Pipeline/masterOutput/output')
...
def hg19Search(margin):
    ...
    if len(xlist) > 0:
        print("\nFollowing gene(s) were found on X chr and will not be searched: " + str(xlist))
    geneDF = geneDF[geneDF.chr != 'X']
    colGenes = list(geneDF['gene_name'])
    errList = list(set(capitalGenes) - set(colGenes))
    if len(errList) > 0:
        print("\nGene(s) not found in HG19_ensembl table: " + str(errList))
    # valGenList = list(set(capitalGenes) - set(colGenes) - set(xlist))
    # print('\n')
    # print(valGenList)
    valGenFile = open("validGeneFile.txt", "w")
...
def sqlSearchString(geneDF, pval):
    stringList = []
    #few lines before while loop print out the error message comparing the string of input from file cast to upper case then compared with genes in the gene dataframe
    #gene dataframe autoskips wrong genes from input, so subtracting input from geneDF column we get our errors
...
def tableLoad(stringList, geneNameList):
    ...
    while j < len(stringList):
            ...
            path = 'E:/BRIAN/Cross_Ref_Pipeline/masterOutput/output/%s.txt' %(geneNameList[j])
