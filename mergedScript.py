...
4. Nothing:\n""")
    ...
    try:
        if option == '1':
            ...
            traits = df.trait
            pVal = df.p_value.min()
            pValLog = df['p_value_log'] = np.log(df['p_value'])
            plotInput = input("What table(s) are you plotting? \nPress ENTER for all...seperate multiple with comma.\n")
            ...
            if len(plotInput)==1:
                ...
                grouped = df.groupby("trait_name")
                multPlot = list(set([int(i) for i in plotInput.split(",")]))
                for i in multPlot:
                    
                    ## Will be completed over weekend.. need to figure out the algorithim to print multiple tables


            # elif plotInput == '':
            #     [i for i in range(1,19)]:
            #         graph = plt.bar(traits, pValLog)  #must loop to show each individually
            #         plt.title('Trait vs P-Value')  
            #         plt.xlabel('Trait')
            #         plt.ylabel('P-Value')
            #         plt.show()  
