...
3. Nothing:\n""")
    try:
        if option == '1':
            path = 'E:/cross_ref/output/cross_ref.txt'
            df.to_csv(path, encoding = 'utf-8', sep='\t',index = False)
            # os.startfile(path)
            choice = input ("name your file:\n")
            os.rename ('E:/cross_ref/output/cross_ref.txt','E:/cross_ref/output/%s.txt' % choice)

        elif option == '2':
            path = 'E:/cross_ref/load/cross_ref.txt'
            df.to_csv(path, encoding = 'utf-8', sep= '\t', index = None)
            # os.startfile(path)
            print("use load_new_table function to load txt file to server")
